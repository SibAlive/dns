...
def catalog_index():
    ...
    for category in categories:
        subcategories = category_service.get_subcategories_by_category_slug(cat_slug=category.slug)
...
def subcategory(category_slug):
    category_service = ProductService(db)
    subcategories = category_service.get_subcategories_by_category_slug(cat_slug=category_slug)

    category_name =  category_service.get_category_by_slug(cat_slug=category_slug).name
    breadcrumbs = [
        {'name': 'DNS', 'endpoint': 'header.index'},
...
def products(subcategory_slug):
    ...
    list_products = category_service.get_products_by_subcategory_slug(subcat_slug=subcategory_slug)
    category = category_service.get_category_by_subcategory_slug(subcat_slug=subcategory_slug)
    category_name = category.name
    category_slug = category.slug
    favorite_ids = cart_service.get_favorites_ids(user_id=current_user.get_id())

    subcategory_name = category_service.get_subcategory_by_slug(slug=subcategory_slug).name
    breadcrumbs = [
...
def product(product_slug):
    ...
    product_card = category_service.get_product_by_slug(product_slug=product_slug)
    favorite_ids = cart_service.get_favorites_ids(user_id=current_user.get_id())

    category = category_service.get_category_slug_by_product_slug(product_slug=product_slug)
    category_slug = category.slug
    category_name = category.name

    subcategory = category_service.get_subcategory_by_product_slug(product_slug=product_slug)
    subcategory_slug = subcategory.slug
    subcategory_name = subcategory.name

    product_name = category_service.get_product_by_slug(product_slug=product_slug).name
    breadcrumbs = [
        {'name': 'DNS', 'endpoint': 'header.index', 'params': {}},
        {'name': 'Каталог', 'endpoint': 'catalog.catalog_index', 'params': {}},
        {'name': category_name, 'endpoint': 'catalog.subcategory', 'params': {'category_slug': category_slug}},
...
def product_sort(subcategory_slug):
    ...
    list_products = category_service.get_products_by_subcategory_slug(
...
def order():
    ...
    if form.validate_on_submit():
        ...
        if not is_phone:
            # Здесь должна быть валидация номера телофона
